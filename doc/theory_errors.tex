...
{\bf Claim: }
\eqn{
\norm{ f^m(x)_i - \tl{f}^m(\tl{x})_i } \le \left( \sigma + \sum^{m - 1}_{i = 1} g^i(\sigma) + g^m(\norm{x - \tl{x}}) \right)_i
}
...
\eqn{
  ...
  &\le g \left(\vec{\sigma} + \sum^{m-2}_{i = 1} g^i(\vec{\sigma}) + g^{m-1}(\vec{\lambda}) \right)+ \vec{\sigma}\\
  &=\sum^{m-1}_{i = 1} g^i(\vec{\sigma}) + g^m(\vec{\lambda}) + \vec{\sigma}
}
