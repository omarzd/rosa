...
class Evaluator(ctx: LeonContext, program: Program, options: RealOptions) {
  ...
  val printStats = true
  private val solver = new RealSolver(ctx, program, options.z3Timeout)

  /* ==========================
      Real evaluation
  ========================== */
  def getRange(precond: Expr, expr: Expr, variables: VariablePool) = {
  ...
  private def inIntervals(expr: Expr, vars: VariablePool): RationalInterval = expr match {
  ...
  /* ==========================
      XFloat evaluation
  ========================== */
  def evalWithError(expr: Expr, precondition: Expr, inputs: VariablePool): (Map[Expr, XFloat], Map[Int, Expr]) = {
    ...
    solver.clearCounts
    val values = inXFloats(expr, variables, config)._1 -- inputs.actualVariables
  ...
  private def evalArith(expr: Expr, vars: Map[Expr, XFloat], config: XFloatConfig): XFloat = {
    val xfloat = expr match {
    case v @ Variable(id) => vars(v)
    case RationalLiteral(v) => XFloat(v, config)
    //case IntLiteral(v) => XFloat(v, config)
    case UMinusF(rhs) => - evalArith(rhs, vars, config)
    case PlusF(lhs, rhs) => evalArith(lhs, vars, config) + evalArith(rhs, vars, config)
    case MinusF(lhs, rhs) => evalArith(lhs, vars, config) - evalArith(rhs, vars, config)
    case TimesF(lhs, rhs) => evalArith(lhs, vars, config) * evalArith(rhs, vars, config)
    case DivisionF(lhs, rhs) => evalArith(lhs, vars, config) / evalArith(rhs, vars, config)
    case SqrtF(t) => evalArith(t, vars, config).squareRoot
