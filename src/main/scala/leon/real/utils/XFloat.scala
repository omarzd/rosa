...
object XFloat {
    ...
    for(rec <- vars) {
      rec match {
        case r @ Record(id, lo, up, Some(absError), aId, _) =>
          val xfloat = xFloatWithUncertain(Variable(id), RationalInterval(lo, up), config,
        ...
        case Record(id, lo, up, None, aId, None) =>
          val xfloat = xFloatWithRoundoff(Variable(id), RationalInterval(lo, up), config, precision)
          variableMap = variableMap + (Variable(aId) -> xfloat)
  ...
  **/
  def xFloatWithRoundoff(v: Variable, range: RationalInterval, config: XConfig, precision: Precision): XFloat = {
    ...
    if (range.isPointRange && isExactInFloats(range.xlo, precision)) {
      ...
      val newError = new RationalForm(Rational.zero) :+ rndoff
      new XFloat(v, range, newError, config, precision)
    }
  ...
  **/
  def xFloatWithUncertain(v: Expr, range: RationalInterval, config: XConfig,
    uncertain: Rational, withRoundoff: Boolean, precision: Precision): XFloat = {
    ...
    if (withRoundoff) {
      ...
      new XFloat(v, range, newError :+ rndoff, config, precision)
    } else {
      new XFloat(v, range, newError, config, precision)
    }
  }
  ...
  /*def withIndex(v: Variable, range: RationalInterval, config: XConfig, precision: Precision): (XFloat, Int) = {
    val rndoff = roundoff(range, precision)
    val (newError, index) = addNoiseWithIndex(new RationalForm(Rational.zero), rndoff)
...
class XFloat(val tr: Expr, val appInt: RationalInterval, val err: RationalForm,
