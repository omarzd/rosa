    ...
    for(rec <- vars) {
      rec match {
        case r @ Record(v @ Variable(_), a @ Variable(_), Some(lo), Some(up), _, _) if (!r.uncertainty.isEmpty) =>
          val (xfloat, index) = XFloat.xFloatWithUncertain(v, RationalInterval(lo, up), config, r.uncertainty.get, withRoundoff, precision)
          variableMap = variableMap + (a -> xfloat)
          indexMap = indexMap + (index -> a)

        case Record(v @ Variable(_), a @ Variable(_), Some(lo), Some(up), None, None) =>
          val (xfloat, index) = XFloat.xFloatWithRoundoff(v, RationalInterval(lo, up), config, precision)
          variableMap = variableMap + (a -> xfloat)
          indexMap = indexMap + (index -> a)
    ...
    for(rec <- vars) {
      rec match {
        case Record(v @ Variable(_), a @ Variable(_), Some(lo), Some(up), _, _) =>
          val xfloat = XFloat.xFloatExact(v, RationalInterval(lo, up), config, precision)
          variableMap = variableMap + (a -> xfloat)
