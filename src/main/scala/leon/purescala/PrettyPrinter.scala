...
class PrettyPrinter(sb: StringBuffer = new StringBuffer) {
  ...
  def pp(tree: Expr, lvl: Int): Unit = tree match {
    ...
    case RelError(expr, n) => ppNary(Seq(expr, n), "relError(",",",")", lvl)
    case Roundoff(expr) => ppUnary(expr, "rndoff(", ")", lvl)
    case Actual(expr) => ppUnary(expr, "act(", ")", lvl)
    case InitialNoise(expr) => ppUnary(expr, "err(", ")", lvl)
    case Sqrt(expr) => ppUnary(expr, "sqrt(", ")", lvl)
    case Assertion(expr) => ppUnary(expr, "assert(", ")", lvl)
    ...
    case Product(exprs) => ppNary(exprs, "(", " ** ", ")", lvl)
    case Power(l, r) => ppBinary(l, r, " ^ ", lvl)
